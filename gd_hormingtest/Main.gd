...
func _process(delta: float) -> void:
	Common.set_trail(_checkbox_laser.button_pressed)
	var max_shot = _hslider_max_shot.value
	_hslider_max_shot_label.text = "%d/32"%max_shot
	Common.set_max_shot(max_shot)
	var is_glow = _checkbox_bloom.button_pressed
	_hdr.environment.glow_enabled = is_glow
	for obj in _bg_layer.get_children():
		if not obj is Sprite2D:
			continue
		var spr = obj as Sprite2D
		if is_glow:
			spr.modulate = Color.SLATE_GRAY
		else:
			spr.modulate = Color.WHITE
	Common.set_smoke(_checkbox_smoke.button_pressed)
	Common.set_smoke_rand(_checkbox_smoke_rand.button_pressed)
